...
function editShop(data) {
  return new Promise(async (resolve, reject) => {
    try {
      let dataInfoShop = await db.Store.findOne({
...
function getDataItems(dataClient) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // All items
      if (
        dataClient.amount === "All" &&
        dataClient.idItems === "EMPTY" &&
        dataClient.idShop === "EMPTY" &&
        dataClient.category === "EMPTY" &&
        dataClient.type === "EMPTY"
      ) {
        // Lấy thông tin items
          ...
          include: [
            {
              model: db.Store,
              as: "storeData",
              attributes: ["manageId", "nameShop"],
            },
            {
              model: db.Discount,
              as: "discountData",
              attributes: ["code", "valueEn", "valueVi"],
            },
            {
              model: db.Category,
              as: "categoryData",
      ...
      if (
        ...
        let dataPriceShip = await db.Ship.findAll({
          ...
          include: [
            {
              model: db.Province,
              attributes: ["keyMap", "type", "id", "valueEn", "valueVi"],
            },
          ],
          raw: true,
          nest: true,
        ...
        let dataPriceShipCategory = await db.Ship.findAll({
          attributes: { exclude: ["createdAt", "id", "updatedAt", "province"] },
          where: {
            category: data.inputType.category,
            itemsId: "EMPTY",
            categoryType: "EMPTY",
          },
          include: [
        ...
        if (allStar) {
          ...
          if (countTotalStart.total && countTotalStart.countStart) {
            countTotalStart.medium =
              countTotalStart.total / countTotalStart.countStart;
          }
        ...
        data.inputType.countOrContentFacebackItems =
          countOrContentFacebackItems;
      ...
      let dataColorImg = await db.Items_color_image.findAll({
        attributes: {
          exclude: ["id", "createdAt", "updatedAt", "imageLink", "color"],
        },
        order: [["createdAt", "DESC"]],
        where: {
          itemId: {
      ...
      let dataSizeAmount = await db.Items_size_amount.findAll({
        attributes: {
          exclude: ["id", "createdAt", "updatedAt", "size", "typeSize"],
        },
        order: [["createdAt", "DESC"]],
        where: {
          itemsId: {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (data.dataItemsColorImgages.length > 0) {
        ...
        // Lấy ra img cũ
        let dataImgOld = [];
        let imgOld = await db.Items_color_image.findAll({
          where: { itemId: data.dataItemsColorImgages[0].itemId },
          attributes: ["image"],
        });
        ...
        // Xóa ảnh cũ
        dataImgOldDelete = dataImgOldDelete.filter(
          (item) => item.trim() !== ""
        );

        dataImgOldDelete.length > 0 &&
          dataImgOldDelete.map((img) => {
      ...
      if (
        data &&
        data.dataImgFile.length === 0 &&
        data.dataItemsColorImgages.length > 0
      ) {
        // Delete data img color
        await db.Items_color_image.destroy({
          where: { itemId: data.dataItemsColorImgages[0].itemId },
      ...
      if (data.dataItems.newPriceUS != "" && data.dataItems.newPrice != "") {
        ...
        price =
          oldDataPrice.newPrice == 0
            ? oldDataPrice.price
            : oldDataPrice.newPrice;
        priceUS =
          oldDataPrice.newPriceUS == 0
            ? oldDataPrice.priceUS
            : oldDataPrice.newPriceUS;
      } else {
    ...
    try {
      ...
      if (_.isEmpty(checkEmptyData)) {
        ...
        if (data.itemsId && data.forItemType !== "" && data.itemsId !== "") {
          // Hàm kiểm tra
          let dataResDiscountItems = await generalHandling.getDiscount(
            data,
            "ITEMSID"
          );
...
function addPriceShip(data) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Loại hàng
      if (
        fristData.category !== "EMPTY" &&
        fristData.categoryType !== "EMPTY" &&
        fristData.itemsId === "EMPTY"
      ) {
        dataExist = await db.Ship.findAll({
          where: {
            idShop: fristData.idShop,
